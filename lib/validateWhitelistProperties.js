 ...
 * If duplicate leaf keys exist at different nesting levels, the later
 * traversal wins.
 *
 * @param {Object} obj
 * @param {Object} [out]
 * @returns {Object}
 */
function flattenObjectByLeafKey(obj, out = {}) {
    for (const [key, value] of Object.entries(obj)) {
        if (value !== null && typeof value === 'object' && !Array.isArray(value) && !(value instanceof Date)) {
            flattenObjectByLeafKey(value, out);
        } else {
            out[key] = value;
        }
    }
 ...
 * same nested parent. This preserves the existing dot-path flatten output for
 * current callers while supporting the sibling-leaf output shape.
 *
 * @param {Array<string[]>} validatedSegments
 * @returns {boolean}
 */
function shouldFlattenByLeafKey(validatedSegments) {
 ...
 *     parent flatten to direct leaf keys. Applied after snake_case conversion.
 *
...
function validateWhitelistProperties(
    ...
    if (flattenOutput) {
        validatedObject = shouldFlattenByLeafKey(validatedSegments)
