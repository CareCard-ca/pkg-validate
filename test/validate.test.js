  ...
  describe('Validate function test', function () {
    it('supports direct and legacy validator calls through the package root', function () {
      ...
      assert.deepStrictEqual(
        validatePackage.validateProperties({ first_name: 'Jane', unexpected: true }),
        {
          first_name: 'Jane',
        },
      );
    ...
    it('isImageUrl returns true if safe image url, false otherwise', function (done) {
      ...
      const image_url_bad_one =
        'A44C0A67*A8704767AD2B97DC46478827/8B57EB9F014143F2AE6B9D8EBBB7CDA6-700.jpg';
      const image_url_bad_two =
        'A44C0A67>A8704767AD2B97DC46478827/8B57EB9F014143F2AE6B9D8EBBB7CDA6-700.jpg';
    ...
    it('isTextString returns true for non-empty text without special character restrictions', function (done) {
      assert.ok(
        validate.isTextString('Requester <tag>@example.com'),
    ...
    it('isUserRoleRequestStatusString returns true only for user role request statuses', function (done) {
      assert.ok(
        validate.isUserRoleRequestStatusString('pending'),
        'pending status should be valid',
      );
      assert.ok(
        validate.isUserRoleRequestStatusString('on_hold'),
        'on_hold status should be valid',
      );
      assert.ok(
        !validate.isUserRoleRequestStatusString('not_a_status'),
        'Unknown status should be rejected',
      );
      assert.ok(
        !validate.isUserRoleRequestStatusString(123),
    ...
    it('isValidTimestampzString returns true if timestamp with time zone, false otherwise', function (done) {
      ...
      assert.ok(
        !validate.isValidTimestampzString(badTz2),
        'Bad timestampz test failed (invalid date)',
      );
      assert.ok(
        !validate.isValidTimestampzString(badTz3),
        'Bad timestampz test failed (missing T)',
      );
    ...
    it('isValidTimestampString returns true if timestamp without time zone, false otherwise', function (done) {
      ...
      assert.ok(
        !validate.isValidTimestampString(badT2),
        'Bad timestamp test failed (invalid date)',
      );
    ...
    it('isValidArrayOfStrings returns true if valid array of strings, false otherwise', function (done) {
      ...
      assert.ok(
        !validate.isValidArrayOfStrings(['a'.repeat(10001)]),
        'array with long string should fail',
      );
      done();
    });
    ...
    it('validateNewUserRoleRequestObject rejects an empty request with default create rules', function (done) {
      assert.throws(
        () => validateNewUserRoleRequestObject(),
        err =>
          err.message === 'Bad_Input' && err.userMessage === 'Missing property: role.institutionId',
      );
      done();
    });
